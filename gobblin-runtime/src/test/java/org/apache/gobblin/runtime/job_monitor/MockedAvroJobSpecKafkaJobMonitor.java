...
@Slf4j
public class MockedAvroJobSpecKafkaJobMonitor extends AvroJobSpecKafkaJobMonitor {
  ...
  public static MockedAvroJobSpecKafkaJobMonitor create(Config config) throws IOException {
    Map<URI, JobSpec> jobSpecs = Maps.<URI, JobSpec>newConcurrentMap();
    return new MockedAvroJobSpecKafkaJobMonitor(config, jobSpecs, createMockCatalogWithDeleteStates(jobSpecs));
  }

  private MockedAvroJobSpecKafkaJobMonitor(Config config, Map<URI, JobSpec> jobSpecs, MutableJobCatalog mockCatalog) throws IOException {
    super("topic", mockCatalog, config, null);
    ...
    try {
      ...
      for (String oneInstruction : SPLITTER_COMMA.split(messageString)) {
        ...
        if (tokens.get(0).equals(REMOVE)) {
          ...
          jobSpec = new JobSpec.Builder(uri).withConfig(ConfigFactory.empty()).
          ...
          jobSpec = new JobSpec.Builder(uri).withConfig(ConfigFactory.empty()).withVersion(version).build();
        }
  ...
  protected static MutableJobCatalog createMockCatalogWithDeleteStates(final Map<URI, JobSpec> jobSpecs) {
    ...
    Mockito.doAnswer(new Answer<Void>() {
      ...
      public Void answer(InvocationOnMock invocation)
          throws Throwable {
        URI uri = ((JobSpec) invocation.getArguments()[0]).getUri();
        jobSpecs.remove(uri);
        return null;
      }
    }).when(jobCatalog).remove(Mockito.any(JobSpec.class));
  ...
  public void awaitExactlyNSpecs(final int n) throws Exception {
    AssertWithBackoff.assertTrue(new Predicate<Void>() {
      ...
      public boolean apply(@Nullable Void input) {
        return MockedAvroJobSpecKafkaJobMonitor.this.jobSpecs.size() == n;
      }
