...
public class DatabaseJobHistoryStore implements JobHistoryStore {
    ...
    private static final String JOB_METRIC_EXIST_QUERY_STATEMENT_TEMPLATE =
            "SELECT * FROM gobblin_job_metrics " +
                    "WHERE job_id=? AND metric_group=? AND metric_name=? AND metric_type=?";

    private static final String TASK_METRIC_EXIST_QUERY_STATEMENT_TEMPLATE =
            "SELECT * FROM gobblin_task_metrics " +
                    "WHERE task_id=? AND metric_group=? AND metric_name=? AND metric_type=?";
        ...
        try {
            ...
            if (jobExecutionInfo.hasMetrics()) {
                for (Metric metric : jobExecutionInfo.getMetrics()) {
                    boolean insert = !existsMetric(connection, JOB_METRIC_EXIST_QUERY_STATEMENT_TEMPLATE,
                            jobExecutionInfo.getJobId(), metric);
                    updateMetric(
                            connection,
                            insert ? JOB_METRIC_INSERT_STATEMENT_TEMPLATE : JOB_METRIC_UPDATE_STATEMENT_TEMPLATE,
                            jobExecutionInfo.getJobId(),
                            metric,
                            insert);
                }
            }
                ...
                for (TaskExecutionInfo info : jobExecutionInfo.getTaskExecutions()) {
                    ...
                    if (info.hasMetrics()) {
                        for (Metric metric : info.getMetrics()) {
                            boolean insert = !existsMetric(connection, TASK_METRIC_EXIST_QUERY_STATEMENT_TEMPLATE,
                                    info.getTaskId(), metric);
                            updateMetric(
                                    connection,
            ...
            switch (query.getIdType()) {
                ...
                case JOB_NAME:
                    return processQueryByJobName(
                        connection, query.getId().getString(), query, Optional.<String>absent());
                case TABLE:
    ...
    private boolean existsJobExecutionInfo(Connection connection, JobExecutionInfo info) throws SQLException {
    ...
    private void insertJobExecutionInfo(Connection connection, JobExecutionInfo info) throws SQLException {
        Preconditions.checkArgument(info.hasJobName());
        Preconditions.checkArgument(info.hasJobId());

        PreparedStatement insertStatement = connection.prepareStatement(
                JOB_EXECUTION_INSERT_STATEMENT_TEMPLATE);
        insertStatement.setString(1, info.getJobName());
        insertStatement.setString(2, info.getJobId());
        insertStatement.setTimestamp(3, info.hasStartTime() ?
                new Timestamp(info.getStartTime()) : Timestamp.valueOf(DEFAULT_TIMESTAMP));
        insertStatement.setTimestamp(4, info.hasEndTime() ?
                new Timestamp(info.getEndTime()) : Timestamp.valueOf(DEFAULT_TIMESTAMP));
    ...
    private void updateJobExecutionInfo(Connection connection, JobExecutionInfo info) throws SQLException {
        Preconditions.checkArgument(info.hasJobId());

        PreparedStatement updateStatement = connection.prepareStatement(
                JOB_EXECUTION_UPDATE_STATEMENT_TEMPLATE);
        updateStatement.setTimestamp(1, info.hasStartTime() ?
                new Timestamp(info.getStartTime()) : Timestamp.valueOf(DEFAULT_TIMESTAMP));
        updateStatement.setTimestamp(2, info.hasEndTime() ?
                new Timestamp(info.getEndTime()) : Timestamp.valueOf(DEFAULT_TIMESTAMP));
    ...
    private boolean existsTaskExecutionInfo(Connection connection, TaskExecutionInfo info) throws SQLException {
        Preconditions.checkArgument(info.hasTaskId());

        PreparedStatement queryStatement = connection.prepareStatement(
                TASK_EXECUTION_EXIST_QUERY_STATEMENT_TEMPLATE);
        queryStatement.setString(1, info.getTaskId());
        return queryStatement.executeQuery().next();
    }

    private void insertTaskExecutionInfo(Connection connection, TaskExecutionInfo info) throws SQLException {
        Preconditions.checkArgument(info.hasTaskId());
        Preconditions.checkArgument(info.hasJobId());

        PreparedStatement insertStatement = connection.prepareStatement(
                TASK_EXECUTION_INSERT_STATEMENT_TEMPLATE);
        ...
        insertStatement.setString(9, info.hasTable() && info.getTable().hasNamespace() ?
                info.getTable().getNamespace() : null);
        insertStatement.setString(10, info.hasTable() && info.getTable().hasName() ?
                info.getTable().getName() : null);
        insertStatement.setString(11, info.hasTable() && info.getTable().hasType() ?
                info.getTable().getType().name() : null);
        insertStatement.executeUpdate();
    }

    private void updateTaskExecutionInfo(Connection connection, TaskExecutionInfo info) throws SQLException {
        Preconditions.checkArgument(info.hasTaskId());

        PreparedStatement updateStatement = connection.prepareStatement(
                TASK_EXECUTION_UPDATE_STATEMENT_TEMPLATE);
        updateStatement.setTimestamp(1, info.hasStartTime() ?
                new Timestamp(info.getStartTime()) : Timestamp.valueOf(DEFAULT_TIMESTAMP));
        updateStatement.setTimestamp(2, info.hasEndTime() ?
                new Timestamp(info.getEndTime()) : Timestamp.valueOf(DEFAULT_TIMESTAMP));
        ...
        updateStatement.setString(7, info.hasTable() && info.getTable().hasNamespace() ?
                info.getTable().getNamespace() : null);
        updateStatement.setString(8, info.hasTable() && info.getTable().hasName() ?
                info.getTable().getName() : null);
        updateStatement.setString(9, info.hasTable() && info.getTable().hasType() ?
                info.getTable().getType().name() : null);
    ...
    private void updateProperty(Connection connection, String template, String id, String key,
                                String value, boolean insert) throws SQLException {
    ...
    private JobExecutionInfo processQueryById(Connection connection, String jobId, Optional<String> tableFilter)
        ...
        PreparedStatement jobExecutionQueryStatement = connection.prepareStatement(
                JOB_EXECUTION_QUERY_BY_JOB_ID_STATEMENT_TEMPLATE);
        ...
        // Query job metrics
        PreparedStatement jobMetricQueryStatement = connection.prepareStatement(
                JOB_METRIC_QUERY_STATEMENT_TEMPLATE);
        ...
        PreparedStatement jobPropertiesQueryStatement = connection.prepareStatement(
        ...
        while (taskRs.next()) {
            ...
            PreparedStatement taskMetricQueryStatement = connection.prepareStatement(
            ...
            PreparedStatement taskPropertiesQueryStatement = connection.prepareStatement(
                    TASK_PROPERTY_QUERY_STATEMENT_TEMPLATE);
    ...
    private List<JobExecutionInfo> processQueryByJobName(Connection connection, String jobName,
        JobExecutionQuery query, Optional<String> tableFilter) throws SQLException {
    ...
    private List<JobExecutionInfo> processQueryByTable(Connection connection, JobExecutionQuery query)
        ...
        ResultSet rs = queryStatement.executeQuery();
        while (rs.next()) {
            jobExecutionInfos.addAll(
                processQueryByJobName(connection, rs.getString(1), query, Optional.of(tableFilter)));
        }
