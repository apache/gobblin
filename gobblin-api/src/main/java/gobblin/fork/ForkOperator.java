...
public interface ForkOperator<S, D> extends Closeable {
   ...
   */
  void init(WorkUnitState workUnitState)
      throws Exception;
   ...
   */
  List<Boolean> forkSchema(WorkUnitState workUnitState, S input);
   ...
   * @return list of {@link java.lang.Boolean}s
   */
  List<Boolean> forkDataRecord(WorkUnitState workUnitState, D input);

  /**
   * Get a list of forked streams.
   */
  default List<Stream<RecordEnvelope<D>>> forkStream(WorkUnitState workUnitState, Stream<RecordEnvelope<D>> inputStream,
      boolean isStreamingTask, List<Boolean> enabledForks) {
    return new ForkedStreams<>(inputStream, workUnitState, this, enabledForks).getOutputStreams();
  }
}
