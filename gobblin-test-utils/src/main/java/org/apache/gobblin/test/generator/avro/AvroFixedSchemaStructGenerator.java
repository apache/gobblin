...
@Slf4j
public class AvroFixedSchemaStructGenerator extends FixedStructValueGenerator<GenericRecord> {
  ...
  ValueGenerator getValueGenerator(FieldConfigWithValueGenerator logicalField, final Schema.Field avroField) {
    // TODO: wrap optional
    final Type type = logicalField.getFieldConfig().getType();
    switch (type) {
      /**
      case Bytes: {
  ...
  public AvroFixedSchemaStructGenerator(FixedSchemaGeneratorConfig config) {
    super(config, AvroValueGeneratorFactory.getInstance());
    ...
    this.valueGenerators = IntStream.range(0, logicalFields.size())
        .mapToObj(i -> getValueGenerator(logicalFieldGenerators.get(i), fields.get(i)))
        .collect(Collectors.toList());
  }

  private Schema.Field getAvroField(FieldConfig requiredField) {
    ...
    switch (requiredField.getOptional()) {
      case OPTIONAL: {
        List<Schema> unionSchema = new ArrayList<Schema>(2);
        unionSchema.add(Schema.create(Schema.Type.NULL));
