...
public class ExponentialBackoff {
   ...
   */
  @Builder(buildMethodName = "awaitCondition", builderMethodName = "await")
  private static boolean evaluateConditionUntilTrue(Callable<Boolean> callable, Double alpha, Integer maxRetries,
      Long maxDelay, Long initialDelay) throws ExecutionException, InterruptedException {
    ExponentialBackoff exponentialBackoff = new ExponentialBackoff(alpha, maxRetries, maxDelay, initialDelay);
    while (true) {
      try {
        if (callable.call()) {
          return true;
