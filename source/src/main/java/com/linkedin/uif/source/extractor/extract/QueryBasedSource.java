...
/**
 * A base implementation of {@link com.linkedin.uif.source.Source} for
 * query-based sources.
 */
public abstract class QueryBasedSource<S, D> extends AbstractSource<S, D> {
    ...
    public List<WorkUnit> getWorkunits(SourceState state) {
        ...
        if (StringUtils.isBlank(extractTableName)) {
            extractTableName = Utils.escapeSpecialCharacters(entityName,
                    ConfigurationKeys.ESCAPE_CHARS_IN_TABLE_NAME, "_");
        }
        ...
        for (Entry<Long, Long> entry : sortedPartitions.entrySet()) {
            ...
            partitionState.setProp(ConfigurationKeys.WORK_UNIT_HIGH_WATER_MARK_KEY,
                    entry.getValue());
    ...
    private long getLatestWatermarkFromMetadata(SourceState state) {
        ...
        for (WorkUnitState workUnitState : previousWorkUnitStates) {
            ...
            if (workUnitState.getWorkingState() == WorkingState.FAILED
                ...
                processedRecordCount = workUnitState
                        .getPropAsLong(ConfigurationKeys.EXTRACTOR_ROWS_EXPECTED);
        ...
        if (isCommitOnFullSuccess && hasFailedRun) {
            ...
            ExtractType extractType = ExtractType.valueOf(previousState.getProp(
                    ConfigurationKeys.SOURCE_QUERYBASED_EXTRACT_TYPE).toUpperCase());

            // add backup seconds only for snapshot extracts but not for appends
            if (extractType == ExtractType.SNAPSHOT) {
                int backupSecs = previousState.getPropAsInt(
                        ConfigurationKeys.SOURCE_QUERYBASED_LOW_WATERMARK_BACKUP_SECS, 0);
                String watermarkType = previousState
                        .getProp(ConfigurationKeys.SOURCE_QUERYBASED_WATERMARK_TYPE);
                latestWaterMark = this.addBackedUpSeconds(previousLowWatermark, backupSecs,
                        watermarkType);
            } else {
        ...
        // tasks.
            ...
            if (isDataProcessedInPreviousRun) {
                ...
                LOG.info("Previous run was successful. Latest watermark - Max watermark from WorkUnitStates: "
                        + latestWaterMark);
            } else {
    ...
    private void initLogger(SourceState state) {
        ...
        sb.append("[");
        sb.append(StringUtils.stripToEmpty(state
                .getProp(ConfigurationKeys.SOURCE_QUERYBASED_SCHEMA)));
        sb.append("_");
