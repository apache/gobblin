...
public class FileBasedExtractor<S, D> implements Extractor<S, D> {
    ...
    public FileBasedExtractor(WorkUnitState workUnitState, FileBasedHelper fsHelper) {
        ...
        this.filesToPull = new ArrayList<String>(workUnitState.getPropAsList(
                ConfigurationKeys.SOURCE_FILEBASED_FILES_TO_PULL, ""));
    ...
    public D readRecord(D reuse) throws DataRecordException, IOException {
        ...
        if (this.totalRecordCount % ConfigurationKeys.DEFAULT_FILEBASED_REPORT_STATUS_ON_COUNT == 0) {
        ...
        if (currentFileItr.hasNext()) {
            if (supportsReuse && seenFirstRecord) {
                try {
                    return (D) currentFileItr.getClass().getMethod("next", reuse.getClass())
                            .invoke(currentFileItr, reuse);
                } catch (Exception e) {
                    e.printStackTrace();
    ...
    public Iterator<D> downloadFile(String file) throws IOException {
        ...
        try {
            ...
            if (workUnitState.getPropAsBoolean(ConfigurationKeys.SOURCE_SKIP_FIRST_RECORD, false)
                    && fileItr.hasNext()) {
                fileItr.next();
            }
            return fileItr;
        } catch (FileBasedHelperException e) {
            throw new IOException("Exception while downloading file " + file + " with message "
                    + e.getMessage(), e);
        }
    }
    ...
    public void closeFile(String file) {
        try {
            ...
            log.error(
                    "Could not successfully close file: " + file + " due to error: "
                            + e.getMessage(), e);
        }
    }
    ...
    public void close() {
        try {
            ...
            log.error(
                    "Could not successfully close file system helper due to error: "
                            + e.getMessage(), e);
        }
    }
}
