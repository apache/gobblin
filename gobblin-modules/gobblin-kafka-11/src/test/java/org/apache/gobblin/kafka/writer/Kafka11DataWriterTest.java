...
@Slf4j
public class Kafka11DataWriterTest {


    private final KafkaTestBase _kafkaTestHelper;
    public Kafka11DataWriterTest()
    ...
    public void testStringSerialization()
        ...
        String topic = "testStringSerialization11";
        ...
        Kafka11DataWriter<String> kafkaDataWriter = new Kafka11DataWriter<String>(props);
    ...
    public void testBinarySerialization()
        ...
        String topic = "testBinarySerialization11";
        _kafkaTestHelper.provisionTopic(topic);
        Properties props = new Properties();
        props.setProperty(KafkaWriterConfigurationKeys.KAFKA_TOPIC, topic);
        props.setProperty(KafkaWriterConfigurationKeys.KAFKA_PRODUCER_CONFIG_PREFIX+"bootstrap.servers", "localhost:" + _kafkaTestHelper.getKafkaServerPort());
        ...
        Kafka11DataWriter<byte[]> kafka11DataWriter = new Kafka11DataWriter<byte[]>(props);
        WriteCallback callback = mock(WriteCallback.class);
        byte[] messageBytes = TestUtils.generateRandomBytes();

        try {
            kafka11DataWriter.write(messageBytes, callback);
        }
        finally
        {
            kafka11DataWriter.close();
        }
    ...
    public void testAvroSerialization()
        ...
        Kafka11DataWriter<GenericRecord> kafka11DataWriter = new Kafka11DataWriter<>(props);
        WriteCallback callback = mock(WriteCallback.class);

        GenericRecord record = TestUtils.generateRandomAvroRecord();
        try {
            kafka11DataWriter.write(record, callback);
        }
        finally
        {
            kafka11DataWriter.close();
