...
public class Kafka10TopicProvisionTest {
  ...
  public void testTopicPartitionCreationCount()
    ...
    Assert.assertEquals(metaData.partitionMetadata().size(), partionCount);
  ...
  public void testLiveTopicPartitionCreationCount()
    ...
    Kafka10DataWriter<String> kafka10DataWriter = new Kafka10DataWriter<String>(props);
    int sessionTimeoutMs = 10 * 1000;
    int connectionTimeoutMs = 8 * 1000;
    // Note: You must initialize the ZkClient with ZKStringSerializer.  If you don't, then
    // createTopic() will only seem to work (it will return without error).  The topic will exist in
    ...
    MetadataResponse.TopicMetadata metaData =
    		AdminUtils.fetchTopicMetadataFromZk(topic,zkUtils);
    Assert.assertEquals(metaData.partitionMetadata().size(), Integer.parseInt(topicPartitionCount));
