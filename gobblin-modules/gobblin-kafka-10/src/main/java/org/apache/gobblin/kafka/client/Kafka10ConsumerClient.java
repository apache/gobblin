 ...
 */
public class Kafka10ConsumerClient<K, V> extends AbstractBaseKafkaConsumerClient {
  ...
  private static final String DEFAULT_GROUP_ID = "kafka10";
  ...
  private Kafka10ConsumerClient(Config config) {
    super(config);
  ...
  public Kafka10ConsumerClient(Config config, Consumer<K, V> consumer) {
    super(config);
  ...
  public Iterator<KafkaConsumerRecord> consume(KafkaPartition partition, long nextOffset, long maxOffset) {
    ...
    return Iterators.transform(consumerRecords.iterator(), new Function<ConsumerRecord<K, V>, KafkaConsumerRecord>() {
      ...
      public KafkaConsumerRecord apply(ConsumerRecord<K, V> input) {
        return new Kafka10ConsumerRecord<>(input);
      }
    });
  }
  ...
  /**
    * A factory class to instantiate {@link Kafka10ConsumerClient}
   */
  public static class Factory implements GobblinKafkaConsumerClientFactory {
    ...
    public GobblinKafkaConsumerClient create(Config config) {
      return new Kafka10ConsumerClient(config);
    }
  }

  /**
   * A record returned by {@link Kafka10ConsumerClient}
   *
   * @param <K> Message key type
   * @param <V> Message value type
   */
  ...
  @ToString
  public static class Kafka10ConsumerRecord<K, V> extends BaseKafkaConsumerRecord implements
      DecodeableKafkaRecord<K, V> {
    private final ConsumerRecord<K, V> consumerRecord;

    public Kafka10ConsumerRecord(ConsumerRecord<K, V> consumerRecord) {
