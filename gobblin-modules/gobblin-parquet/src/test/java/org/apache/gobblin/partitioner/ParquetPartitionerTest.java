...
public class ParquetPartitionerTest {
  ...
  public void setUp() {
    ...
    List<Type> types = asList(typeForProductName, typeForProductId, typeForRegionId, typeForCost);
    schemaForRecords = new GroupType(REQUIRED, "products", types);
  }

  private ParquetPartitioner createPartitioner(String partKey, String partDefaultValue) {
    ...
    when(state.getProp(partKeyName)).thenReturn(partKey);
    when(state.getProp(defPartKeyName, DEFAULT_WRITER_PARQUET_PARTITION_KEY_DEFAULT)).thenReturn(partDefaultValue);
    return new ParquetPartitioner(state, NUM_OF_BRANCHES, BRANCH_ID);
  }

  @Test
  public void testParquetGroupsPartitionedByAKey() {
    partitioner = createPartitioner("region_id", "NA");
    ParquetGroup record1 = buildRecord("MacBook", 1, 5, 500.90);
    ParquetGroup record2 = buildRecord("MacBook Air", 5, 5, 5090.90);
    ParquetGroup record3 = buildRecord("MacBook Air2", 6, 1, 5090.90);
    ParquetGroup record4 = buildRecord("MacBook Air3", 7, 1, 5090.90);
    ParquetGroup record5 = buildRecord("MacBook Air4", 8, 10, 5090.90);
  ...
  public void testExceptionOnPartitionKeyNotFoundInParquetSchema() {
    partitioner = createPartitioner("non_existing_key", "NA");
    ParquetGroup record1 = buildRecord("MacBook", 1, 5, 500.90);
  ...
  private ParquetGroup buildRecord(String productName, int productId, int regionId, double cost) {
