  ...
  public void run()
    ...
    for (final Dataset dataset : datasets) {
      ListenableFuture<Void> future = this.service.submit(new Callable<Void>() {
        ...
        public Void call()
            throws Exception {
          if (dataset instanceof ValidableDataset) {
            ((ValidableDataset) dataset).validate();
          } else {
            log.warn(
                "Not an instance of " + ValidableDataset.class + " Dataset won't be validated " + dataset.datasetURN());
          }
          return null;
        }
      });
