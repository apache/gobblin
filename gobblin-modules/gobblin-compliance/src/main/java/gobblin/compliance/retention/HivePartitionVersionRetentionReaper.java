...
public class HivePartitionVersionRetentionReaper extends HivePartitionVersionRetentionRunner {
  ...
  public void clean()
    ...
    this.backUpOnwerFs = ProxyUtils.getOwnerFs(state, this.backUpOwner);
  ...
  private void executeAlterQueries(HiveProxyQueryExecutor queryExecutor)
    ...
    String dbName = version.getDbName();
    ...
    try {
      ...
      queryExecutor.executeQuery(HivePurgerQueryTemplate
              .getCreateTableQuery(backUpDb + "." + backUpTableName, dbName, tableName, backUpTableLocation),
          this.backUpOwner);
  ...
  private void executeDropVersionQueries(HiveProxyQueryExecutor queryExecutor)
    ...
    try {
      queryExecutor.executeQuery(getUseDbQuery(dbName), this.versionOwner);
      queryExecutor.executeQuery(getDropPartitionQuery(tableName, partitionSpec), this.versionOwner);
    } catch (SQLException e) {
      throw new IOException(e);
    }
  }
  ...
  private String getBackUpTableLocation(HivePartitionVersion version) {
    ...
    return this.state.getProp(ComplianceConfigurationKeys.BACKUP_DIR) + "/" + getCompleteTableName(version);
  }

  private Path getNewVersionLocation() {
    HivePartitionVersion version = (HivePartitionRetentionVersion) this.datasetVersion;
    String backUpTableLocation = getBackUpTableLocation(version);
    return new Path(backUpTableLocation + "/" + getVersionTimeStamp() + "/" + version.getName());
  }
}
