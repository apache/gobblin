...
public class PostgresWriterCommands implements JdbcWriterCommands {
  private static final Logger LOG = LoggerFactory.getLogger(PostgresWriterCommands.class);
  ...
  public PostgresWriterCommands(State state, Connection conn) {
    this.conn = conn;
    // Reuse the MysqlBufferedInserter for Postgres as well.
  ...
  public Map<String, JdbcType> retrieveDateColumns(String database, String table) throws SQLException {
    Map<String, JdbcType> targetDataTypes = ImmutableMap.<String, JdbcType> builder()
        ...
        .put("TIME WITH TIME ZONE", JdbcType.TIME)
        .put("TIME WITHOUT TIME ZONE", JdbcType.TIME)
        .put("TIMESTAMP WITH TIME ZONE", JdbcType.TIMESTAMP)
        .put("TIMESTAMP WITHOUT TIME ZONE", JdbcType.TIMESTAMP)
        .build();

    ImmutableMap.Builder<String, JdbcType> dateColumnsBuilder = ImmutableMap.builder();

    try (PreparedStatement pstmt = this.conn.prepareStatement(INFORMATION_SCHEMA_SELECT_SQL_PSTMT, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY)) {
