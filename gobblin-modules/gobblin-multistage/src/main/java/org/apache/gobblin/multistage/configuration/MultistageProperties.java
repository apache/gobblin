...
public enum MultistageProperties {
  ...
  MSTAGE_HTTP_CLIENT_FACTORY("ms.http.client.factory", String.class) {
    @Override
    public <T> T getDefaultValue() {
      return (T) "org.apache.gobblin.multistage.factory.GaapHttpClientFactory";
    }
  },
  /**
   ...
   * - the name specifies the field in response that gives session info.
   * - the condition, specifies when the session should stop.
   * - the failCondition specifies when the session should fail.
   *
   * A conditions can be a regular expression or a formula. Currently, only regular expression is supported.
   *
   * "name" is required
   ...
   * 1. a dataset URN, if the source schema can be represented by a dataset.
   *    For example: urn:li:dataset:(urn:li:dataPlatform:hive,rightnow.incidents,PROD)
   *    In such case, the latest schema of the dataset will be read from metadata store,
   *    and then parsed to retrieve fields and types, etc
   * 2. a registered schema, if the source schema is registered with metadata store in
  ...
  MSTAGE_SOURCE_SCHEMA_READER_FACTORY("ms.source.schema.reader.factory", String.class) {
    @Override
    public <T> T getDefaultValue() {
      return (T) "org.apache.gobblin.multistage.factory.MetastoreFactory";
    }
  },
  /**
  ...
  /**
   * Work unit partitioning scheme is either a string or a JsonObject.
   *
  ...
  /**
   * get property value for a specified MultistageProperties item
   * <p>
   * This version serves those Source properties, the duplication is not ideal, we could
   * make this better by define the getProp methods on State, instead of WorkUnitState and
   * SourceState separately in Gobblin core.
   ...
   * @return the property value of the specific MultistageProperties item
   */
...
// END of enum MultistageProperties