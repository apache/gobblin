...
public class ADFPipelineExecutionTaskTest extends PowerMockTestCase {
  ...
  public void testBuildUri()
    ...
    wu.setProp(ADFPipelineExecutionTask.AZURE_SUBSCRIPTION_ID, "123");
    String aadId = "456";
    wu.setProp(ADFPipelineExecutionTask.AZURE_ACTIVE_DIRECTORY_ID, aadId);
    wu.setProp(ADFPipelineExecutionTask.AZURE_RESOURCE_GROUP_NAME, "rg1");
    wu.setProp(ADFPipelineExecutionTask.AZURE_DATA_FACTORY_NAME, "df1");
    wu.setProp(ADFPipelineExecutionTask.AZURE_DATA_FACTORY_PIPELINE_NAME, "p1");
    wu.setProp(ADFPipelineExecutionTask.AZURE_DATA_FACTORY_API_VERSION, "v1");
    String akvUrl = "akv-url";
    wu.setProp(ADFPipelineExecutionTask.AZURE_KEY_VAULT_URL, akvUrl);
    String akvExecId = "akv_exec_id";
    wu.setProp(ADFPipelineExecutionTask.AZURE_SERVICE_PRINCIPAL_ADF_EXECUTOR_ID, akvExecId);
    wu.setProp(ADFPipelineExecutionTask.AZURE_SERVICE_PRINCIPAL_KEY_VAULT_READER_ID, "akv_reader_id");
    wu.setProp(ADFPipelineExecutionTask.AZURE_SERVICE_PRINCIPAL_KEY_VAULT_READER_SECRET, "akv_reader_secret");
    wu.setProp(ADFPipelineExecutionTask.AZURE_KEY_VAULT_SECRET_ADF_EXEC, "adf_exec_secret_name");
    wu.setProp(ADFPipelineExecutionTask.AZURE_DATA_FACTORY_PIPELINE_PARAM,
        "{\"date\":\"2019-10-15\",\"customerName\":\"chen\",\"password\":\"qwert12345!\",\"securityToken\":\"12345\"}");
    ...
    PowerMockito
        .stub(PowerMockito.method(KeyVaultSecretRetriever.class, "getSecret", String.class, String.class, String.class))
        .toReturn(new SecretBundle().withValue(fetchedAdfExecToken));

    CachedAADAuthenticator cachedAADAuthenticator = mock(CachedAADAuthenticator.class);
    when(cachedAADAuthenticator
        .getToken(AADTokenRequesterImpl.TOKEN_TARGET_RESOURCE_MANAGEMENT, akvExecId, fetchedAdfExecToken))
        .thenReturn(CachedAADAuthenticatorTest.url1s1sp1token2);
    PowerMockito.mockStatic(CachedAADAuthenticator.class);

    PowerMockito
        .stub(PowerMockito.method(CachedAADAuthenticator.class, "getToken", String.class, String.class, String.class))
        .toReturn(CachedAADAuthenticatorTest.url1s1sp1token2);

    when(CachedAADAuthenticator.buildWithAADId(aadId, wu.getProperties())).thenReturn(cachedAADAuthenticator);
    HttpUriRequest httpRequest = task.createHttpUriRequest();
    Assert.assertEquals(httpRequest.getURI().toString(),
        "https://management.azure.com/subscriptions/123/resourceGroups/rg1/providers/Microsoft.DataFactory/factories/df1/pipelines/p1/createRun?api-version=v1");
    ...
    Assert.assertEquals(httpRequest.getHeaders("Authorization")[0].getValue(),
        "Bearer " + CachedAADAuthenticatorTest.url1s1sp1token2AccessToken);
    HttpEntity entity = ((HttpPost) httpRequest).getEntity();
    Map<String, String> map = JsonUtils.jsonToMap(EntityUtils.toString(entity));
  ...
  public void testJsonToMap1() {
    Map<String, String> map = JsonUtils.jsonToMap("{\"k1\":\"v1\",\"k2\":\"v2\"}");
  ...
  public void testJsonToMap2() {
    Map<String, String> map = JsonUtils.jsonToMap("");
