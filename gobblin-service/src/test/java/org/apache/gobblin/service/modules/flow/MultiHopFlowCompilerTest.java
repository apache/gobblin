  ...
  public void testCompileFlowWithRetention() throws URISyntaxException, IOException {
    ...
    for (int i = 0; i < 9; i += 2) {
      ...
      jobNames.add(Joiner.on(JobExecutionPlan.Factory.JOB_NAME_COMPONENT_SEPARATION_CHAR).
          join(flowGroup, flowName, expectedJobNames.get(i), (Joiner.on(':').join(sourceNodes.get(i), destinationNodes.get(i), edgeNames.get(i)))));
      if (i < 8) {
        jobNames.add(Joiner.on(JobExecutionPlan.Factory.JOB_NAME_COMPONENT_SEPARATION_CHAR).
            join(flowGroup, flowName, expectedJobNames.get(i + 1), (Joiner.on(':').join(sourceNodes.get(i + 1), destinationNodes.get(i + 1), edgeNames.get(i + 1)))));
      }
