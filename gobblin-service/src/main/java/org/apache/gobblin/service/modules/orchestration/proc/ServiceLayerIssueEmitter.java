 ...
 *
 * <p>Flow-level issues (jobName = "NA") are for errors occurring before any job exists or affecting
 * the entire flow. Job-level issues are for errors tied to a specific job.
 */
@Slf4j
public final class ServiceLayerIssueEmitter {
  ...
  public static void emitFlowIssue(EventSubmitter eventSubmitter, String flowGroup, String flowName,
      String flowExecutionId, IssueSeverity severity, String code, String summary, String details) {
    emitIssue(eventSubmitter, flowGroup, flowName, flowExecutionId, JobStatusRetriever.NA_KEY,
        severity, code, summary, details);
  }

  /**
   * Emit a flow-level issue using {@link Dag.DagId} for convenience.
   */
  public static void emitFlowIssue(EventSubmitter eventSubmitter, Dag.DagId dagId,
      IssueSeverity severity, String code, String summary, String details) {
    emitFlowIssue(eventSubmitter, dagId.getFlowGroup(), dagId.getFlowName(),
        String.valueOf(dagId.getFlowExecutionId()), severity, code, summary, details);
  }
   ...
   */
  public static void emitJobIssue(EventSubmitter eventSubmitter, String flowGroup, String flowName,
      String flowExecutionId, String jobName, IssueSeverity severity, String code,
      String summary, String details) {
    emitIssue(eventSubmitter, flowGroup, flowName, flowExecutionId, jobName,
        severity, code, summary, details);
  }

  /**
   * Emit a job-level issue using {@link Dag.DagId} for convenience.
   */
  public static void emitJobIssue(EventSubmitter eventSubmitter, Dag.DagId dagId, String jobName,
      IssueSeverity severity, String code, String summary, String details) {
    emitJobIssue(eventSubmitter, dagId.getFlowGroup(), dagId.getFlowName(),
        String.valueOf(dagId.getFlowExecutionId()), jobName, severity, code, summary, details);
  }

  private static void emitIssue(EventSubmitter eventSubmitter, String flowGroup, String flowName,
      String flowExecutionId, String jobName, IssueSeverity severity, String code,
      String summary, String details) {
    try {
      ...
      log.debug("Emitted service-layer issue: code={}, severity={}, jobName={}, summary={}",
          code, severity, jobName, summary);
    } catch (Exception e) {
      log.error("Failed to emit service-layer issue: code={}, summary={}", code, summary, e);
    }
  }
}
