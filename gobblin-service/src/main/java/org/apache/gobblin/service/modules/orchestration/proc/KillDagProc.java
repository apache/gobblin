...
@Slf4j
@Alpha
public final class KillDagProc extends DagProc<Optional<KillDagProc.CancelEntity>, Optional<Dag<JobExecutionPlan>>> {
  ...
  protected Optional<CancelEntity> initialize(DagManagementStateStore dagManagementStateStore) throws IOException {
    Dag<JobExecutionPlan> dagToCancel = dagManagementStateStore.getDag(this.killDagTask.getDagId().toString());
    List<Dag.DagNode<JobExecutionPlan>> dagNodesToCancel = dagManagementStateStore.getDagNodes(this.killDagTask.getDagId().toString());
    return Optional.of(new CancelEntity(dagToCancel, dagNodesToCancel));
  }

  @Override
  public Optional<Dag<JobExecutionPlan>> act(DagManagementStateStore dagManagementStateStore, Optional<CancelEntity> cancelEntity) throws IOException {
    if (!cancelEntity.isPresent()) {
      log.warn("No dag with id " + this.killDagTask.getDagId() + " found to kill");
      return Optional.empty();
    }
    for (Dag.DagNode<JobExecutionPlan> dagNodeToCancel : cancelEntity.get().dagNodesToCancel) {
      cancelDagNode(cancelEntity.get().dagToCancel, dagNodeToCancel, dagManagementStateStore);
    }
    return Optional.of(cancelEntity.get().dagToCancel);
  }
  ...
  private void cancelDagNode(Dag<JobExecutionPlan> dagToCancel, Dag.DagNode<JobExecutionPlan> dagNodeToCancel, DagManagementStateStore dagManagementStateStore) throws IOException {
    ...
    try {
      ...
      dagManagementStateStore.deleteDagAction(this.killDagTask.getDagAction());
  ...
  static class CancelEntity {
    private final Dag<JobExecutionPlan> dagToCancel;
    private final List<Dag.DagNode<JobExecutionPlan>> dagNodesToCancel;
  }
}
