 ...
 */
@Slf4j
public class KillDagProc extends DagProc<Optional<Dag<JobExecutionPlan>>, Optional<Dag<JobExecutionPlan>>> {
  private final Optional<DagNodeId> dagNodeId;

  public KillDagProc(KillDagTask killDagTask) {
    this.dagTask = killDagTask;
  ...
  protected Optional<Dag<JobExecutionPlan>> act(DagManagementStateStore dagManagementStateStore, Optional<Dag<JobExecutionPlan>> dag)
    ...
    if (this.dagNodeId.isPresent()) {
      cancelDagNode(dagManagementStateStore.getDagNode(this.dagNodeId.get()).get());
      return dag;
    }
    ...
    // todo - where are other type of dag actions removed?
