...
public class InfluxDBJobMetricsStore implements JobMetricsStore {
    ...
    public InfluxDBJobMetricsStore(Properties properties) {
        ...
        this.influxDB = InfluxDBFactory.connect(
                influxUrl,
                properties.getProperty(ConfigurationKeys.FLUXDB_USER_NAME_KEY,
                        ConfigurationKeys.DEFAULT_FLUXDB_USER_NAME),
                properties.getProperty(ConfigurationKeys.FLUXDB_USER_PASSWORD_KEY,
                        ConfigurationKeys.DEFAULT_FLUXDB_USER_PASSWORD));
    }

    @Override
    public void put(JobMetrics metrics) throws IOException {
        ...
        for (Map.Entry<String, Metric> metric : metricMap.entrySet()) {
            ...
            this.influxDB.write(
                    ...
                    new Serie.Builder(metrics.getJobName())
                            .columns(metricNames.toArray(new String[metricNames.size()]))
                            .values(metricValues.toArray(new Object[metricValues.size()]))
                            .build());
        }
    }

    @Override
    public Map<Long, Object> get(String jobName, JobMetrics.MetricGroup metricGroup,
                                 String id, String name, MetricNameSuffix suffix)
            throws IOException {
        ...
        String query = String.format("select %s, %s from %s where %s='%s'",
                TIME_KEY, metricName, jobName, idKey, id);
    ...
    @Override
    public Object aggregateOnTask(String jobName, String taskId,
                                  String name, MetricNameSuffix suffix,
                                  AggregationFunction aggFunction)
            throws IOException {

        String aggAlias = name + suffix.getSuffix() + aggFunction.name();
        String query = String.format("select %s(%s) as %s from %s where %s='%s'",
                aggFunction.name(), name + suffix.getSuffix(), aggAlias, jobName,
                TASK_ID_KEY, taskId);
    ...
    @Override
    public Map<String, Object> aggregateOnTasks(String jobName, String jobId,
                                                String name, MetricNameSuffix suffix,
                                                AggregationFunction aggFunction)
            throws IOException {
        ...
        String aggAlias = name + suffix.getSuffix() + aggFunction.name();
        String query = String.format("select %s, %s(%s) as %s from %s where %s='%s' group by %s",
                TASK_ID_KEY, aggFunction.name(), name + suffix.getSuffix(), aggAlias, jobName,
                JOB_ID_KEY, jobId, TASK_ID_KEY);
        List<Serie> series = this.influxDB.query(TASK_METRICS_DB, query, TimeUnit.MILLISECONDS);
    ...
    @Override
    public Map<String, Object> aggregateOnJobRuns(String jobName, String name,
        ...
        String aggAlias = name + suffix.getSuffix() + aggFunction.name();
        String query = String.format("select %s, %s(%s) as %s from %s group by %s",
                JOB_ID_KEY, aggFunction.name(), name + suffix.getSuffix(), aggAlias,
                jobName, JOB_ID_KEY);
    ...
    @Override
    public Object aggregateOnJob(String jobName, String name, MetricNameSuffix suffix,
                                 AggregationFunction aggFunction) throws IOException {

        String aggAlias = name + suffix.getSuffix() + aggFunction.name();
        String query = String.format("select %s(%s) as %s from %s",
                aggFunction.name(), name + suffix.getSuffix(), aggAlias, jobName);
        List<Serie> series = this.influxDB.query(JOB_METRICS_DB, query, TimeUnit.MILLISECONDS);
    ...
    @Override
    public Object aggregateOnJobs(Collection<String> jobNames,
                                  String name, MetricNameSuffix suffix,
                                  AggregationFunction aggFunction)
            throws IOException {
        ...
        String aggAlias = name + suffix.getSuffix() + aggFunction.name();
        String query = String.format("select %s(%s) as %s from %s",
                aggFunction.name(), name + suffix.getSuffix(), aggAlias,
