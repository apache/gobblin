            ...
            for (int i = 0; i < branches; i++) {
                ...
                if (this.fss.get(i).exists(publisherOutput)) {
                    ...
                    if (!replaceFinalOutputDir) {
                        ...
                        boolean preserveFileName = workUnitState.getPropAsBoolean(
                                ForkOperatorUtils.getPropertyNameForBranch(
                                        ConfigurationKeys.SOURCE_FILEBASED_PRESERVE_FILE_NAME, branches, i),
                                false);
                        for (FileStatus status : this.fss.get(i).listStatus(writerOutput)) {
                            ...
                            Path outputPath = preserveFileName ?
                                    new Path(publisherOutput, workUnitState.getProp(
                                            ForkOperatorUtils.getPropertyNameForBranch(
