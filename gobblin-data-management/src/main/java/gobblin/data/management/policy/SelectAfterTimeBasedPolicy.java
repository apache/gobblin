...
/**
 * An implementation of {@link AbstractTimeBasedSelectionPolicy} which returns versions that are newer than a certain time.
 */
public class SelectAfterTimeBasedPolicy extends AbstractTimeBasedSelectionPolicy {
  ...
  protected Predicate<TimestampedDatasetVersion> getSelectionPredicate() {
    return new Predicate<TimestampedDatasetVersion>() {
      ...
      public boolean apply(TimestampedDatasetVersion version) {
        return version.getDateTime().plus(SelectAfterTimeBasedPolicy.this.lookBackPeriod).isAfterNow();
      }
    };
  }
}
