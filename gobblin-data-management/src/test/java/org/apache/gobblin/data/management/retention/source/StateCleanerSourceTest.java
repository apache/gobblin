...
public class StateCleanerSourceTest {
  ...
  private String getSourcePrefixed(String string) {
    return StateCleanerSource.STATE_CLEANER_SOURCE_PREFIX + "." + string;
  }
  ...
  public void testSingleConfig() {
    StateCleanerSource source = new StateCleanerSource();
    ...
    Assert.assertEquals(workUnits.get(0).getProp(TASK_FACTORY_KEY), StateCleanerTaskFactory.class.getName());
  ...
  public void testMultipleConfig() {
    ...
    props.put(StateCleanerSource.STATE_CLEANER_CONFIGURATIONS, "config1, config2");
    ...
    Assert.assertEquals(workUnits.get(0).getProp(TASK_FACTORY_KEY), StateCleanerTaskFactory.class.getName());
    Assert.assertEquals(workUnits.get(1).getProp(TASK_FACTORY_KEY), StateCleanerTaskFactory.class.getName());
