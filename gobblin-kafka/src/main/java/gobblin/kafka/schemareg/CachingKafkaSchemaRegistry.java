...
public class CachingKafkaSchemaRegistry<K,S> implements KafkaSchemaRegistry<K,S> {
  ...
  public CachingKafkaSchemaRegistry(KafkaSchemaRegistry kafkaSchemaRegistry, int maxSchemaReferences)
  {
    ...
    if (kafkaSchemaRegistry.hasInternalCache())
    {
      log.warn(new StringBuilder().append("The passed in KafkaSchemaRegistry ")
          .append(kafkaSchemaRegistry.getClass().getCanonicalName())
          .append(" already has a cache. Putting another cache on top isn't going to make things faster!").toString());
    }
