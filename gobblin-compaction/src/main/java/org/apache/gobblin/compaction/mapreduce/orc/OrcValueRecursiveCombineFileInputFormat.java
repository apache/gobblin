...
// TODO: Remove code duplication from Avro version.
public class OrcValueRecursiveCombineFileInputFormat extends CombineFileInputFormat<NullWritable, OrcValue> {
  ...
  // TODO: Verify if this is useful anymore.
  private void setSplitSize(JobContext cx) {
    super.setMaxSplitSize(cx.getConfiguration()
        .getLong(COMPACTION_JOB_MAPRED_MAX_SPLIT_SIZE, DEFAULT_COMPACTION_JOB_MAPRED_MAX_SPLIT_SIZE));
    super.setMinSplitSizeNode(cx.getConfiguration()
        .getLong(COMPACTION_JOB_MAPRED_MIN_SPLIT_SIZE, DEFAULT_COMPACTION_JOB_MAPRED_MIN_SPLIT_SIZE));
  }
  ...
  private static List<InputSplit> cleanSplits(List<InputSplit> splits) throws IOException {
    ...
    for (int i = 0; i < splits.size(); i++) {
      ...
      cleanedSplits.add(
          new CombineFileSplit(oldSplit.getPaths(), oldSplit.getStartOffsets(), oldSplit.getLengths(), locations));
    }
