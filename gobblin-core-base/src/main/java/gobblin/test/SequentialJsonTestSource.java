 ...
 */
@Slf4j
public class SequentialJsonTestSource implements Source<String, JsonObject> {
  ...
  public Extractor<String, JsonObject> getExtractor(WorkUnitState state)
    ...
    return new Extractor<String, JsonObject>() {
      ...
      @Override
      public JsonObject readRecord(@Deprecated JsonObject reuse)
        ...
        if (recordsExtracted < numRecordsPerExtract) {
          ...
          obj.addProperty("seq", currentWatermark.getValue());
          return obj;
        }
        else
        {
