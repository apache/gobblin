    ...
    for (Field field : schema.getFields()) {
      if (!this.shouldRemove(field)) {
        ...
        if (this.children.containsKey(field.name())) {
          newField = AvroCompatibilityUtils.newField(field.name(),
            this.children.get(field.name()).removeFields(field.schema(),
              schemaMap), field.doc(), AvroCompatibilityUtils.fieldDefaultValue(field));
        } else {
          newField = AvroCompatibilityUtils.newField(field.name(),
            DO_NOTHING_INSTANCE.removeFields(field.schema(), schemaMap),
              field.doc(), AvroCompatibilityUtils.fieldDefaultValue(field));
        }
        for (Map.Entry<String, Object> stringJsonNodeEntry : field.getObjectProps().entrySet()) {
